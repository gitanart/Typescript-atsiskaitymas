...
{
    const solution = (arr) => arr[arr.length - 1];
    console.log({ numbers, result: solution(numbers) });
    console.log({ strings, result: solution(strings) });
    console.log({ booleans, result: solution(booleans) });
}
...
{
    const solution = (value, count) => Array.from(new Array(count)).map(() => value);
    const dataSamples = [
        ['a', 2],
        [77, 4],
        [true, 1],
...
{
    const solution = (arr1, arr2) => [...arr1, ...arr2];
